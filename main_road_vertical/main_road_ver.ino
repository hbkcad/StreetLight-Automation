...
Serial.println(val); 
  if(ctrl_var == 0)
  {
    ...
    if(event.light>=700)
    {
     ...
     if(now.hour()>=9 && !(int(digitalRead(IRSens1Pin))) && !(int(digitalRead(IRSens2Pin))) && count!=1){
     ...
     // {
      ...
      
      {
        ...
        analogWrite(STLight11,250);delay(500);
        analogWrite(STLight10,250);
        analogWrite(STLight9,250);delay(500);
